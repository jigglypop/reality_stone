# 그래프 디퓨전·리만 라플라시안·DP 메모리 LLM 컨텍스트 이론
...
### 2.3 라플라스‑벨트라미 기반 학습 해석

열 방정식

$$
\partial_\tau x = \Delta_g x
$$

은 디리클레 에너지

$$
E[x] = \tfrac{1}{2} \int_M \langle \nabla x, \nabla x \rangle_g \, d\mathrm{vol}_g
$$

를 줄이는 그래디언트 흐름으로 해석할 수 있다.
따라서 RS‑ULF에서의 항 $\alpha \Delta_g x$ 는

- 표현 $x$ 를 기하에 맞게 부드럽게 만들고
- 고주파 잡음과 기하와 불일치하는 방향을 억제하며
- 라플라스‑벨트라미 스펙트럼의 저차 모드 위로 정보를 투사하도록

하는 “라플라스 식을 이용한 학습” 역할을 수행한다.

---

## 3. DP/Bellman 메모리 $V_t$ 이론
...
## 5. 퀀트/Multivariate 시나리오에서의 해석 이론
