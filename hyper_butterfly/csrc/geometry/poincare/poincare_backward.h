...
#ifdef WITH_CUDA
// 역전파 함수 선언 (CUDA)
std::vector<torch::Tensor> log_map_backward_cuda(
    torch::Tensor grad_output,
    torch::Tensor x,
    float c);

std::vector<torch::Tensor> exp_map_backward_cuda(
    torch::Tensor grad_output,
    torch::Tensor v,
    float c);
#endif
...
public:
    // 로그 맵 역전파
    static std::vector<torch::Tensor> log_map_backward(
        torch::Tensor grad_output,
        torch::Tensor x,
        float c) {
    ...
    // 지수 맵 역전파
    static std::vector<torch::Tensor> exp_map_backward(
        torch::Tensor grad_output,
        torch::Tensor v,
        float c) {
