...
__global__ void log_map_kernel(
    ...
    if ((tid & (warpSize - 1)) == 0) atomicAdd(s_norm2, local);
    __syncthreads();
    // 2) clamp & factor
    if (tid == 0)  s_norm2[0] = fmaxf(s_norm2[0], EPS);
    __syncthreads();
...
torch::Tensor log_map_cuda(torch::Tensor x, float c) {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        x.scalar_type(), "log_map_cuda", [&] { log_map_kernel<scalar_t> << <B, threads, shbytes >> > (
            x.data_ptr<scalar_t>(),
...
torch::Tensor exp_map_cuda(torch::Tensor v, float c) {
    ...
    AT_DISPATCH_FLOATING_TYPES(
        v.scalar_type(), "exp_map_cuda", [&] { exp_map_kernel<scalar_t> << <B, threads, shbytes >> > (
            v.data_ptr<scalar_t>(),
            out.data_ptr<scalar_t>(),
            c, B, D); });
