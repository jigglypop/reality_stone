"""CLI for BA-TR20 one-shot source-event composition."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.stage.startswith("generate-"):
        ...
        if args.input is None:
            parser.error("--input is required for one-shot analysis")
        result = analyze_one_shot_artifact(args.input, stage=args.stage)
