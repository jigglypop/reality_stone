"""CLI for BA-TR22 three-event relevance no-go."""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.stage.startswith("generate-"):
        ...
        if args.input is None:
            parser.error("--input is required for relevance analysis")
        result = analyze_relevance_no_go_artifact(args.input, stage=args.stage)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(json.dumps({key: value for key, value in result.items() if key != "rows"}, sort_keys=True))
    return 0 if result["status"].endswith(("READY", "PASS", "CONFIRMED")) else 1
