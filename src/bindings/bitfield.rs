...
use crate::bitfield::{BitfieldLayout, BitfieldLinear as RustBitfieldLinear};
...
impl PyBitfieldLinear {
    #[new]
    fn new(m: usize, n: usize, basis_size: usize, r_max: f32) -> Self {
        Self {
            inner: RustBitfieldLinear::new(m, n, basis_size, r_max, BitfieldLayout::Standard32Bit),
            bias: None,
        }
    }
    ...
    /// 잔차 데이터 반환 (있으면)
    pub fn get_residual_data<'py>(&self, _py: Python<'py>) -> PyResult<Option<PyObject>> {
        // 잔차 데이터를 딕셔너리로 반환
    ...
    pub fn forward_cuda_direct(
        ...
        let buffers = self.inner.get_gpu_buffers().ok_or_else(|| {
            PyErr::new::<pyo3::exceptions::PyRuntimeError, _>("GPU not initialized")
        })?;
    ...
    pub fn backward_cuda_direct(
        ...
        if in_features != self.inner.get_n() || out_features != self.inner.get_m() {
            return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(format!(
                "Dimension mismatch in backward: expected ({}, {}), got ({}, {})",
                self.inner.get_n(),
                self.inner.get_m(),
                in_features,
                out_features
    ...
    pub fn forward(&mut self, input: PyObject) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            ...
            let _device_type = device.getattr(py, "type")?.extract::<String>(py)?;
            ...
            let result_array_dyn: Array<f32, IxDyn> = if x_arr.ndim() == 2 {
                let x_2d = x_arr
                    .to_owned()
                    .into_shape((x_arr.shape()[0], x_arr.shape()[1]))
                    .unwrap();
    ...
    pub fn backward(&self, grad_output: PyObject) -> PyResult<PyObject> {
        Python::with_gil(|py| {
            ...
            let grad_input_dyn: Array<f32, IxDyn> = if grad_arr.ndim() == 2 {
                let grad_2d = grad_arr
                    .to_owned()
                    .into_shape((grad_arr.shape()[0], grad_arr.shape()[1]))
                    .unwrap();
    ...
    pub fn backward_cuda(
        ...
        let batch_size = shape[0];
        let grad_output_slice = unsafe {
            slice::from_raw_parts(
                grad_output_ptr as *const f32,
                batch_size * self.inner.get_m(),
            )
        };
        let grad_output =
            ArrayView::from_shape((batch_size, self.inner.get_m()), grad_output_slice).unwrap();
