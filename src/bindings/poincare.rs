use crate::{create_binding, layers::poincare, ops::mobius, ops::project};
use numpy::{IntoPyArray, PyArray1, PyArray2, PyReadonlyArray2};
use pyo3::prelude::*;

// --- 매크로를 사용한 바인딩 생성 ---

create_binding!(poincare_distance_cpu, poincare::poincare_distance, [u, v, c], PyArray1);
create_binding!(poincare_to_lorentz_cpu, poincare::poincare_to_lorentz, [x, c], PyArray2);
create_binding!(poincare_to_klein_cpu, poincare::poincare_to_klein, [x, c], PyArray2);
...
pub fn poincare_ball_layer_backward_cpu<'py>(
    ...
    let (grad_u, grad_v) = poincare::poincare_ball_layer_backward(
        &grad_output.as_array(), &u.as_array(), &v.as_array(), c, t
    );
...
pub fn mobius_add_vjp_cpu<'py>(
    ...
    let (grad_x, grad_y) = poincare::mobius_add_vjp(
        &grad_output.as_array(), &x.as_array(), &y.as_array(), c
    );
...
pub fn mobius_scalar_vjp_cpu<'py>(
    ...
    poincare::mobius_scalar_vjp(
        &grad_output.as_array(), &x.as_array(), c, r
    ).into_pyarray(py)
}
