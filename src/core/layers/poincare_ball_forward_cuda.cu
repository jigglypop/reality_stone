...
namespace reality_stone::layers {
    ...
    torch::Tensor poincare_ball_forward_cuda(torch::Tensor u, torch::Tensor v, float c, float t) {
        ...
        // 하이퍼볼릭 연산 수행 (CUDA)
        auto minus_u = ops::mobius_scalar_cuda(u, safe_c, -1.0f);
        minus_u = utils::safe_clamp_tensor(minus_u);
        
        auto delta = ops::mobius_add_cuda(minus_u, v, safe_c);
        delta = utils::safe_clamp_tensor(delta);
        
        auto delta_t = ops::mobius_scalar_cuda(delta, safe_c, safe_t);
        delta_t = utils::safe_clamp_tensor(delta_t);
        
        auto result = ops::mobius_add_cuda(u, delta_t, safe_c);
