...
impl GeodesicMemory {
            ...
            if !self.buffer.is_empty() {
                 ...
                 if t > (cp_last.t as f32) && t <= (*t_last as f32) {
                      ...
                      let v_tip = if self.buffer.len() >= 2 {
                           &self.buffer[self.buffer.len()-1].1 - &self.buffer[self.buffer.len()-2].1
                      } else {
                           Array1::zeros(self.d_model)
                      };
    ...
    pub fn get_compression_stats(&self) -> (usize, usize, f32) {
        ...
        let ratio = if stored > 0 { covered as f32 / stored as f32 } else { 0.0 };
...
mod tests {
    ...
    fn test_spline_compression_sine_wave() {
        ...
        println!("Compression: Stored {} / Covered {} (Ratio {:.2}x)", stored, covered, ratio);
        ...
        println!("Reconstruction Error: Avg MSE {:.6}, Max MSE {:.6}", avg_mse, max_mse);
