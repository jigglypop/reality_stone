use _rust::layers::rsulf::{
    analyze_layer, compute_curvature, create_causal_laplacian, create_compression_plan,
    extract_global_basis, ffn_force_and_grad_row, fold_dimension_svd, fold_ffn_svd,
    verify_compression_plan, verify_fold_consistency, RSULFConfig, RSULFLayer,
};
...
fn rsulf_벨만_메모리_감쇠() {
    ...
    assert!(v > 0.0 && v < 1.0, "V_t 범위 이상: {}", v);

    let mut v_decay = 1.0_f32;
    for _ in 0..100 {
        v_decay = gamma * v_decay;
    }
    assert!(v_decay < 0.5, "gamma^100 감쇠 실패: {}", v_decay);
}
...
fn rsulf_forward_출력_유한() {
    ...
    assert!(v_new.iter().all(|v| v.is_finite()), "v_new에 NaN/Inf 포함");
}
...
fn rsulf_그래프_디퓨전_효과() {
    ...
    assert!(lx.iter().all(|v| v.is_finite()), "L*x에 NaN/Inf 포함");
...
fn rsulf_지오데식_스텝_근사() {
    ...
    assert!(diff > 0.0, "곡률 보정이 효과 없음");
    assert!(diff < 1.0, "곡률 보정이 너무 큼: diff={}", diff);
}
...
fn rsulf_압축률_계산() {
    ...
    assert!(ratio > 1.0, "압축률 1 미만: ratio={}", ratio);
}

#[test]
fn rsulf_폴딩_정확도_임계값_소형() {
    let test_cases = vec![(64, 32), (64, 16), (64, 8), (128, 64), (128, 32), (128, 16)];
...
fn rsulf_폴딩_정확도_임계값_대형() {
    ...
    let test_cases = vec![(256, 128), (256, 64), (256, 32)];
...
fn rsulf_ffn_포텐셜_그래디언트_수치_검증() {
    ...
    for i in 0..2 {
        ...
        let diff = (&grad_analytic - &grad_num)
            .mapv(|z| z.abs())
            .mean()
            .unwrap();
...
fn rsulf_지수맵_근사_정확도() {
    ...
    let wq = Array2::<f32>::from_shape_fn((d, d), |(i, j)| if i == j { 1.0 } else { 0.0 });
    let wk = Array2::<f32>::from_shape_fn((d, d), |(i, j)| if i == j { 1.0 } else { 0.0 });
...
fn rsulf_메트릭_대칭_반대칭_분해() {
    ...
    let recon_err: f32 = (&b - &reconstructed)
        .iter()
        .map(|v| v * v)
        .sum::<f32>()
        .sqrt();
...
fn rsulf_global_basis_extraction_test() {
    ...
    for _ in 0..num_layers {
        wqs.push(Array2::<f32>::from_shape_fn((d, d), |_| {
            rand::random::<f32>() * 0.1
        }));
        wks.push(Array2::<f32>::from_shape_fn((d, d), |_| {
            rand::random::<f32>() * 0.1
        }));
    }
