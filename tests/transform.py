...
def convert_to_equivalent_hyperbolic(model: nn.Module, c: float = 1.0):
    ...
    for name, module in model.named_children():
        ...
        # 임베딩 레이어를 교체
            ...
            poincare_emb = PoincareEmbedding.from_euclidean_embedding(module, c=c)
            equiv_layer = EquivalentPoincareEmbedding.from_poincare_embedding(poincare_emb)
            setattr(model, name, equiv_layer)
            conversion_count += 1
...
def main():
    ...
    # 첫 번째 임베딩 레이어 확인
    if hasattr(original_model, 'transformer') and hasattr(original_model.transformer, 'wte'):
        orig_wte = original_model.transformer.wte.weight[0, :5].detach()
        conv_wte = converted_model.transformer.wte.embedding.weight[0, :5].detach()
        print(f"  Original wte[0,:5]: {orig_wte}")
        print(f"  Converted wte[0,:5]: {conv_wte}")
        print(f"  Difference: {torch.abs(orig_wte - conv_wte).max().item():.6f}")
    
    # 첫 번째 Linear 레이어 확인
    if hasattr(original_model, 'lm_head'):
        orig_weight = original_model.lm_head.weight[0, :5].detach()
        conv_weight = converted_model.lm_head.euclidean_weight[0, :5].detach()
        print(f"  Original lm_head[0,:5]: {orig_weight}")
        print(f"  Converted lm_head[0,:5]: {conv_weight}")
        print(f"  Difference: {torch.abs(orig_weight - conv_weight).max().item():.6f}")
